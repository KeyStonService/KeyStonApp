# 執行與偵錯系統 (Run &amp; Debug System)

## 🎯 系統概述

這是一個超越傳統 IDE 的執行與偵錯系統，整合了：
- **多語言支援**：Python, JavaScript/TypeScript, Go, Rust, Java 等
- **終端機偵錯**：直接在終端機中執行偵錯命令
- **互動式聊天偵錯**：使用自然語言進行偵錯
- **智能診斷**：AI 驅動的錯誤分析與修復建議
- **launch.json 相容**：完全相容 VS Code 的配置格式

## 🏗️ 系統架構

```
┌─────────────────────────────────────────────────────────────┐
│                    使用者介面層                              │
├──────────────┬──────────────────┬──────────────────────────┤
│  終端機介面   │   聊天介面        │   VS Code 擴充           │
└──────────────┴──────────────────┴──────────────────────────┘
                         │
┌─────────────────────────────────────────────────────────────┐
│                    執行與偵錯引擎                            │
├──────────────┬──────────────────┬──────────────────────────┤
│  執行管理器   │   偵錯協議        │   會話管理               │
└──────────────┴──────────────────┴──────────────────────────┘
                         │
┌─────────────────────────────────────────────────────────────┐
│                    語言適配層                                │
├──────────────┬──────────────────┬──────────────────────────┤
│  Python DAP  │   Node.js DAP     │   其他語言 DAP           │
└──────────────┴──────────────────┴──────────────────────────┘
                         │
┌─────────────────────────────────────────────────────────────┐
│                    底層執行環境                              │
├──────────────┬──────────────────┬──────────────────────────┤
│  程序管理     │   檔案系統        │   網路通訊               │
└──────────────┴──────────────────┴──────────────────────────┘
```

## 🔧 核心功能

### 1. 執行管理 (Execution Management)

#### 1.1 配置系統
支援標準 launch.json 格式，範例配置：

**Python 配置**
```json
{
  "type": "python",
  "request": "launch",
  "name": "Python: Current File",
  "program": "${file}",
  "console": "integratedTerminal"
}
```

**Node.js 配置**
```json
{
  "type": "node",
  "request": "launch",
  "name": "Node: Current File",
  "program": "${file}",
  "skipFiles": ["<node_internals>/**"]
}
```

#### 1.2 變數替換
- `${workspaceFolder}` - 工作區根目錄
- `${file}` - 當前檔案
- `${fileBasename}` - 當前檔案名稱
- `${fileDirname}` - 當前檔案目錄
- `${env:NAME}` - 環境變數
- `${config:NAME}` - 配置值

### 2. 偵錯協議 (Debug Adapter Protocol)

完整實作 DAP 協議，支援所有標準偵錯操作。

### 3. 終端機偵錯介面

#### 3.1 命令列工具
```bash
# 啟動偵錯會話
$ mno debug start --config python-debug

# 設定斷點
$ mno debug breakpoint set src/main.py:10

# 執行到下一行
$ mno debug next

# 檢視變數
$ mno debug var inspect myVariable

# 評估表達式
$ mno debug eval "x + y"

# 檢視堆疊追蹤
$ mno debug stack

# 繼續執行
$ mno debug continue
```

#### 3.2 互動式偵錯 REPL
```bash
$ mno debug repl
(mno-debug) break src/main.py:10
Breakpoint 1 set at src/main.py:10

(mno-debug) run
Starting program...
Breakpoint 1 hit at src/main.py:10

(mno-debug) print x
x = 42

(mno-debug) next
Stepped to src/main.py:11

(mno-debug) continue
Program finished.
```

### 4. 聊天式偵錯介面

#### 4.1 自然語言互動範例

**錯誤診斷**
```
User: 為什麼我的程式在第 10 行崩潰？

AI: 我分析了您的程式碼，發現第 10 行有除以零的錯誤。
    變數 'divisor' 的值為 0，導致 ZeroDivisionError。
    
    建議修復：
    1. 在除法前檢查 divisor 是否為 0
    2. 使用 try-except 捕獲異常
    
    要我幫您自動修復嗎？
```

**變數檢查**
```
User: x 的值是多少？

AI: 在當前斷點處，變數 x 的值為：
    x = 42 (type: int)
    
    相關變數：
    - y = 10
    - result = x + y = 52
```

**程式碼建議**
```
User: 如何優化這段程式碼？

AI: 我發現以下優化機會：
    1. 第 15-20 行的迴圈可以用列表推導式簡化
    2. 第 25 行的字串拼接可以用 f-string 提升效能
    3. 第 30 行的條件判斷可以合併
    
    要我展示優化後的程式碼嗎？
```

### 5. 智能診斷系統

#### 5.1 錯誤分析
- 自動識別常見錯誤模式
- 提供上下文相關的修復建議
- 學習專案特定的錯誤模式

#### 5.2 效能分析
- 即時效能監控
- 記憶體使用分析
- 瓶頸識別與建議

#### 5.3 程式碼品質檢查
- 靜態分析整合
- 最佳實踐建議
- 安全性漏洞檢測

## 📚 使用範例

### 範例 1：Python 偵錯

**launch.json**
```json
{
  "version": "0.2.0",
  "configurations": [
    {
      "name": "Python: Debug Current File",
      "type": "python",
      "request": "launch",
      "program": "${file}",
      "console": "integratedTerminal",
      "justMyCode": true,
      "env": {
        "PYTHONPATH": "${workspaceFolder}/src"
      }
    }
  ]
}
```

**終端機使用**
```bash
$ mno debug start --config "Python: Debug Current File"
$ mno debug break src/calculator.py:15
$ mno debug run
# 程式在斷點處停止
$ mno debug var inspect result
$ mno debug next
$ mno debug continue
```

### 範例 2：Node.js 偵錯

**launch.json**
```json
{
  "version": "0.2.0",
  "configurations": [
    {
      "name": "Node: Debug Server",
      "type": "node",
      "request": "launch",
      "program": "${workspaceFolder}/src/server.js",
      "skipFiles": ["<node_internals>/**"],
      "env": {
        "NODE_ENV": "development"
      }
    }
  ]
}
```

**聊天式偵錯**
```
User: 啟動 Node 伺服器偵錯

AI: 已啟動 Node.js 偵錯會話。伺服器正在 http://localhost:3000 運行。
    要設定斷點嗎？

User: 在處理 /api/users 請求的地方設定斷點

AI: 已在 src/routes/users.js:25 設定斷點。
    這是處理 GET /api/users 請求的位置。
    
User: 發送測試請求

AI: 已觸發斷點！當前狀態：
    - req.params = {}
    - req.query = { page: 1, limit: 10 }
    - 即將執行資料庫查詢
    
    要繼續執行還是檢查變數？
```

### 範例 3：多語言專案偵錯

**launch.json**
```json
{
  "version": "0.2.0",
  "configurations": [
    {
      "name": "Full Stack Debug",
      "type": "compound",
      "configurations": [
        "Python: Backend API",
        "Node: Frontend Dev Server"
      ]
    },
    {
      "name": "Python: Backend API",
      "type": "python",
      "request": "launch",
      "program": "${workspaceFolder}/backend/main.py",
      "console": "integratedTerminal"
    },
    {
      "name": "Node: Frontend Dev Server",
      "type": "node",
      "request": "launch",
      "runtimeExecutable": "npm",
      "runtimeArgs": ["run", "dev"],
      "cwd": "${workspaceFolder}/frontend"
    }
  ]
}
```

## 🚀 進階功能

### 1. 條件斷點
```bash
$ mno debug break src/main.py:10 --condition "x > 100"
```

### 2. 日誌點 (Logpoints)
```bash
$ mno debug logpoint src/main.py:15 --message "Value of x: {x}"
```

### 3. 資料斷點 (Data Breakpoints)
```bash
$ mno debug watch myObject.property
```

### 4. 異常斷點
```bash
$ mno debug break-on-exception --type ValueError
```

### 5. 時間旅行偵錯 (Time Travel Debugging)
```bash
$ mno debug record
$ mno debug replay
$ mno debug step-back
```

## 🔌 擴充性

### 自訂語言支援
```typescript
import { DebugAdapter } from '@mno/debug-core';

class MyLanguageDebugAdapter extends DebugAdapter {
  async initialize(args: InitializeRequestArguments) {
    // 實作初始化邏輯
  }
  
  async launch(args: LaunchRequestArguments) {
    // 實作啟動邏輯
  }
  
  // 實作其他 DAP 方法...
}

// 註冊適配器
registerDebugAdapter('mylang', MyLanguageDebugAdapter);
```

### 自訂診斷規則
```yaml
# custom-diagnostics.yaml
rules:
  - id: custom-null-check
    pattern: "if x == None:"
    message: "使用 'if x is None:' 而不是 'if x == None:'"
    severity: warning
    fix: "if x is None:"
    
  - id: custom-performance
    pattern: "for .+ in .+:\n\\s+list.append"
    message: "考慮使用列表推導式以提升效能"
    severity: info
```

## 📊 監控與分析

### 偵錯會話統計
- 斷點命中次數
- 平均偵錯時間
- 最常見的錯誤類型
- 效能瓶頸分析

### 團隊協作
- 共享偵錯配置
- 偵錯會話錄製與回放
- 協作式問題解決

## 🔒 安全性

### 敏感資料保護
- 自動遮蔽密碼和金鑰
- 安全的環境變數處理
- 偵錯日誌加密

### 存取控制
- 基於角色的偵錯權限
- 生產環境偵錯限制
- 審計日誌

## 📖 最佳實踐

1. **使用有意義的配置名稱**
2. **善用條件斷點減少中斷次數**
3. **利用日誌點進行非侵入式偵錯**
4. **定期清理未使用的斷點**
5. **使用聊天介面快速診斷問題**
6. **記錄並分享有用的偵錯配置**

## 🎓 學習資源

- [快速入門指南](./RUN_DEBUG_QUICKSTART.md)
- [API 參考文件](./RUN_DEBUG_API.md)
- [故障排除指南](./RUN_DEBUG_TROUBLESHOOTING.md)
- [範例專案](../examples/debug-examples/)
- [影片教學](https://example.com/tutorials)

## 🤝 貢獻

歡迎貢獻新的語言支援、診斷規則或功能改進！
請參閱 [貢獻指南](../CONTRIBUTING.md)。

## 📝 授權

MIT License - 詳見 [LICENSE](../LICENSE) 檔案。